...
* [`src/clj/reframe_codenames/`](src/clj/reframe_codenames/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/reframe_codenames/`](src/cljs/reframe_codenames/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/reframe_codenames/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `reframe_codenames` project root directory
...
in the [`src/clj/reframe_codenames/`](src/clj/reframe_codenames/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_codenames/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-codenames.example
  (:require [reframe-codenames.config :as config])
